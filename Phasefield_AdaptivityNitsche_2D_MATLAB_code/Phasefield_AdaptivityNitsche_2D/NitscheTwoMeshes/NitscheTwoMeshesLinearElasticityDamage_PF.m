function [K,f] = NitscheTwoMeshesLinearElasticityDamage_PF(X,T,Xref,Tref,NitscheFaces,refinedElements,referenceElementStd,referenceElementRef,E_elems,nu_elems,eta,beta,d)
...
for i = 1:length(standardElements)
    ...
    Te = T(iElem,:); Xe = X(Te,:);
    de = d(Te);
    Ke = computeVolumeMatrices(Xe,referenceElementStd,E,nu,eta,de);
    ind = [Te,Te+nOfNodes];
...
for i = 1:length(refinedElements)
    ...
    de = d(Te+size(X,1));
    Ke = computeVolumeMatrices(Xe,referenceElementRef,E,nu,eta,de);
...
%computed as if both adjacent elements were refined + projection to standard
%space
